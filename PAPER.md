...
Files for code: 
- [VAE/resnet_vae.py](VAE/resnet_vae.py) ResNet-like VAE architecture
...
The training loop is managed using PyTorch Lightning's Trainer class, which automates key tasks like device management, checkpointing, and logging. The entire process is highly parallelized, using multiple data loader workers and pinned memory to minimize data transfer bottlenecks during model training. A bf16-mixed precision was used to speed up training while reducing memory usage on my GPU.

Files for code:
- [VAE/main.py](VAE/main.py)

#### Hurdles and Lesson Learned
...
Although the final outputs are not yet convincing enough to fool someone into believing they are part of the training dataset, the amount of structure the model can reconstruct from the latent space is impressive. 
...
- The generator starts by mapping a latent vector of size 128 (or another configurable size) to a learned feature map using a fully connected layer.

- The feature map is reshaped into a 2D tensor and passed through a series of upsampling blocks.
...
The ResNet-inspired versions generally yielded better results due to their improved stability and ability to model complex features in the data.
...
- The loss function varied depending on the discriminator's loss. BCE between the discriminators prediction and labels of "real" in the case of BCE loss chosen and the negative mean prediction in the case of Wasserstein and hinge loss. These loss functions motivate the generator to produce images the discriminator would classify as real. The best loss was found to be hinge loss in the experiments.
...
The lengthy training times were primarily caused by the additional overhead of training the discriminator multiple times per batch (--disc_ratio) and the computational cost introduced by loss calculations such as hinge loss and Wasserstein loss with gradient penalties. Each epoch took considerable time, making rapid iteration difficult and requiring a more organized approach to debugging.
...
By epoch 100, the images generated by the GAN closely resemble Kanji, with some even appearing realistic enough to be mistaken for input images. The lines are well-formed and structured, and the outputs clearly capture the overall essence of Kanji characters.

I am very happy with these results, especially considering how quickly they were achieved after resolving the initial issues with the training configuration. While I did attempt further iterations and adjustments to improve the GAN's performance, the results did not improve significantly beyond this point. I believe the model reached an effective balance between the generator and discriminator early on, and further modifications yielded diminishing returns.

### Denoising Diffusion Probabilistic Model (DDPM)
#### Overview
The Denoising Diffusion Probabilistic Model (DDPM) is a generative model that progressively transforms random noise into coherent images through an iterative denoising process. During training, Gaussian noise is applied to input images incrementally across a fixed number of timesteps, degrading the images into pure noise. The model is trained to predict and remove the noise at each step in this process. When generating new images, the DDPM works in reverse, starting with pure noise and iteratively refining it by predicting and removing noise, step by step, until a coherent image emerges.
...
- The decoder symmetrically mirrors the encoder, progressively increasing spatial resolution (through bilinear interpolation) while reducing the number of channels (512 → 256 → 128 → 64).
...
- A final convolutional layer maps the feature representation back to the original image dimensions with img_channels channels (grayscale for Kanji). A Tanh activation ensures output pixel values lie in the range [-1, 1].

This U-Net design makes the model well-suited for predicting and remove noise effectively at each timestep. The sinusoidal embeddings provide critical temporal information, ensuring the model understands which timestep it is operating on.
...
- The Adam optimizer is used with a learning rate of 1e-4, ensuring stable gradient updates.

- Learning rate scheduling is handled by a CosineAnnealingLR scheduler, which gradually reduces the learning rate over the course of training to prevent overfitting. This technique was crucial in improving the stability of samples.
...
#### Hurdles and Lessons Learned
Training the DDPM presented several challenges, particularly in achieving stability with the generated images. One recurring issue was the model producing high-quality images during training, only to revert back to random noise a few epochs later. After careful experimentation, I determined that the optimizer may have been making overly aggressive parameter updates, which destabilized the training process. Introducing a cosine annealing learning rate scheduler resolved this issue, allowing the model to converge more smoothly and produce consistently stable samples by the end of training.
...
Inspired by an example repository I was referencing, I attempted to enhance the model by incorporating triplet attention mechanisms between each block in the U-Net. While implementing this feature was an interesting exercise, it did not result in noticeable improvements to the model's performance. This highlighted the importance of evaluating architectural changes thoroughly rather than relying on complex additions to improve results.
...
By epoch 25, Kanji-like shapes start to emerge in some samples, though the results remain inconsistent. Some denoising processes yield promising images with visible structure, while others fail to remove enough noise to produce meaningful results.
...
- GAN:
    - The GAN produces the cleanest and most visually convincing Kanji-like images, with well-defined lines and strong structural resemblance to the training data. Some generated characters are realistic enough to be mistaken for input images. However, occasional artifacts and imbalances between the generator and discriminator can still hinder consistency across outputs.
...
- VAE:
    - The VAE was by far the easiest to implement and train, with stable loss dynamics and straightforward debugging. It provided immediate results within hours of training and required minimal tuning, making it a great stepping stone into more complex generative models.
...
- GAN:
    - The GAN’s architecture is relatively polished, but further tuning could be explored—such as balancing generator/discriminator updates with an adaptive schedule based on discriminator accuracy.

- DDPM:
    - The DDPM shows the most promise for future work. Removing residual noise would dramatically improve the image quality, and experimenting with attention mechanisms, alternative architectural designs, or other sampling method could help achieve this goal. Additionally, increasing the number of timesteps during sampling or further optimizing the cosine schedules might lead to more consistent results.
